...
export default function StudentChatScreen() {
  ...
  const loadRooms = useCallback(async () => {
    try {
      const [roomsRes, contactsRes] = await Promise.allSettled([
        api.get('/chat'),
        api.get('/students/me/contacts'),
      ]);
  ...
  const handleStartChat = useCallback(async (contactUserId: string) => {
    try {
      const res = await api.post(`/chat/direct/${contactUserId}`);
      const chatId = res.data?.id || res.data?.data?.id;
      await loadRooms();
      const updatedRes = await api.get('/chat');
      const updatedRooms: ChatRoom[] = Array.isArray(updatedRes.data) ? updatedRes.value : [];
      const room = updatedRooms.find((r) => r.id === chatId);
      if (room) handleSelectRoom(room);
    } catch {
      Alert.alert('Erro', 'Não foi possível iniciar a conversa.');
    }
  }, [loadRooms, handleSelectRoom]);
  ...
  // Contacts that don't have a chat yet
  const existingChatUserIds = new Set(
    rooms.flatMap((r) => r.participants?.map((p) => p.userId) || [])
  );
  const contactsWithoutChat = contacts.filter((c) => !existingChatUserIds.has(c.userId));

  if (selectedRoom) {
    const otherUser = getOtherUser(selectedRoom);
    const first = otherUser?.profile?.firstName;
    const last = otherUser?.profile?.lastName;
    const otherName = `${first || ''} ${last || ''}`.trim() || 'Usuário';
    const otherInitials = getInitials(first, last);
    const roleLabel = ROLE_LABELS[otherUser?.role || ''] || '';
          ...
          <View style={styles.chatHeaderInfo}>
            ...
            <View style={styles.chatStatusRow}>
              ...
              <Text style={[styles.chatStatus, { color: connected ? COLORS.success : COLORS.muted }]}>
                {connected ? 'Online' : (roleLabel || 'Offline')}
              </Text>
            </View>
          </View>
        </LinearGradient>
  ...
  return (
    <View style={styles.container}>
      <LinearGradient colors={['#6f5cf020', '#06b6d420']} style={styles.header}>
        ...
        <Text style={styles.subtitle}>Fale com seus profissionais</Text>
      </LinearGradient>
      ...
      {loadingRooms ? (
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.roomsList}>
          {/* Contacts without existing chat */}
          {contactsWithoutChat.length > 0 && (
            <View style={styles.contactsSection}>
              <Text style={styles.sectionLabel}>SEUS PROFISSIONAIS</Text>
              {contactsWithoutChat.map((contact, idx) => {
                const initials = getInitials(contact.firstName, contact.lastName);
                const name = `${contact.firstName || ''} ${contact.lastName || ''}`.trim() || 'Profissional';
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() => handleStartChat(contact.userId)}
                    activeOpacity={0.8}
                  >
                    ...
                    <Text style={styles.startChatLabel}>Iniciar</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
          ...
          {rooms.length > 0 && (
            <>
              {contactsWithoutChat.length > 0 && (
                <Text style={[styles.sectionLabel, { marginTop: 8, marginBottom: 8 }]}>CONVERSAS</Text>
              )}
          ...
          {rooms.length === 0 && contactsWithoutChat.length === 0 && (
            <View style={styles.centered}>
              <Text style={styles.emptyIcon}>💬</Text>
              <Text style={styles.emptyText}>Nenhuma conversa ainda</Text>
              <Text style={styles.emptySubtext}>Seu trainer ou nutricionista irá entrar em contato com você aqui</Text>
            </View>
          )}
        </ScrollView>
      )}
...
const styles = StyleSheet.create({
  ...
  startChatLabel: { color: COLORS.primary, fontSize: 12, fontWeight: '700' },
  contactRole: { color: COLORS.accent, fontSize: 11, marginTop: 2 },
  roomCard: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: COLORS.card,
  ...
  roomLastMsg: { color: COLORS.muted, fontSize: 13, marginTop: 3 },
