...
export class StudentsService {
  ...
  async getWorkoutPlan(userId: string) {
    ...
    // Mantém a estrutura completa do WorkoutPlan + Workout + Trainer para o frontend
    return plans.map((p) => ({
      id: p.id,
      ...
      workout: {
        ...
        level: p.workout.level,
        status: p.workout.status,
        isTemplate: p.workout.isTemplate,
        tags: p.workout.tags,
        trainerId: p.workout.trainerId,
        trainer: p.workout.trainer ? {
          id: p.workout.trainer.id,
          userId: p.workout.trainer.userId,
          cref: p.workout.trainer.cref,
          specialties: p.workout.trainer.specialties,
          user: p.workout.trainer.user ? {
            id: p.workout.trainer.user.id,
            email: p.workout.trainer.user.email,
            profile: p.workout.trainer.user.profile ? {
              firstName: p.workout.trainer.user.profile.firstName,
              lastName: p.workout.trainer.user.profile.lastName,
              avatarUrl: p.workout.trainer.user.profile.avatarUrl,
              phone: p.workout.trainer.user.profile.phone,
            } : null,
          } : null,
        } : null,
