...
export default function StudentDiet() {
  ...
  function downloadDietPDF() {
    ...
    const nutriProfile = diet.nutritionist?.user?.profile;
    const professionalLine = nutriProfile
      ? [nutriProfile.firstName, nutriProfile.lastName].filter(Boolean).join(' ')
      : undefined;
    const ok = openDietPDF(diet, { professionalLine });
