...
export default function NutritionistPayments() {
  ...
  const { data: pricing } = useQuery({
    queryKey: ['nutritionist-pricing'],
    queryFn: () => api.get('/nutritionist-billing/nutritionist/pricing').then((r) => r.data?.data ?? r.data),
  });
  ...
  const { data: billings = [], isLoading: billingsLoading } = useQuery({
    queryKey: ['nutritionist-billings'],
    queryFn: () => api.get('/nutritionist-billing/nutritionist/billings').then((r) => r.data?.data ?? r.data),
  });
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ['nutritionist-billings'] });

  const setPricingMut = useMutation({
    mutationFn: (body: any) => api.post('/nutritionist-billing/nutritionist/pricing', body),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['nutritionist-pricing'] }),
  });

  const subscribeMut = useMutation({
    mutationFn: ({ userId, interval, dueDate }: any) =>
      api.post(`/nutritionist-billing/nutritionist/patients/${userId}/subscribe`, { interval, dueDate }),
    onSuccess: () => { invalidate(); setSubscribeModal(null); },
  });

  const markPaidMut = useMutation({
    mutationFn: (invoiceId: string) => api.patch(`/nutritionist-billing/nutritionist/invoices/${invoiceId}/mark-paid`, {}),
    onSuccess: invalidate,
  });

  const editInvoiceMut = useMutation({
    mutationFn: ({ id, ...body }: any) => api.patch(`/nutritionist-billing/nutritionist/invoices/${id}`, body),
    onSuccess: () => { invalidate(); setEditInvoiceModal(null); },
  });

  const deleteInvoiceMut = useMutation({
    mutationFn: (id: string) => api.delete(`/nutritionist-billing/nutritionist/invoices/${id}`),
    onSuccess: () => { invalidate(); setDeleteInvoiceId(null); },
  });

  const cancelBillingMut = useMutation({
    mutationFn: (id: string) => api.delete(`/nutritionist-billing/nutritionist/billings/${id}`),
  ...
  const toggleAccessMut = useMutation({
    mutationFn: ({ id, released, note }: { id: string; released: boolean; note?: string }) =>
      api.patch(`/nutritionist-billing/nutritionist/billings/${id}/access`, { released, note }),
    onSuccess: (_, { released }) => {
      invalidate();
      toast.success(released ? 'Acesso do paciente liberado' : 'Acesso do paciente bloqueado');
    },
  ...
  const unbilledPatients = patientsArr.filter((s: any) => !billedUserIds.has(s.userId));
  ...
  function exportCSV() {
    const rows = [
      ['Paciente', 'Plano', 'Valor (R$)', 'Status', 'Próx. vencimento'],
      ...arr.map((b: any) => [
        b.studentName,
  ...
  const summaryCards = [
    ...
    { label: 'Ativos', value: active.length, icon: Users, color: 'from-purple-600 to-indigo-600', sub: 'pacientes em dia' },
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Gerencie mensalidades e planos dos seus pacientes</p>
        </div>
      ...
      {tab === 'cobrancas' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass-card">
          <h2 className="font-semibold mb-4">Pacientes e Cobranças</h2>
          {billingsLoading ? (
          ...
          ) : arr.length === 0 && unbilledPatients.length === 0 ? (
            ...
            <div className="space-y-2">
              ...
              {arr.map((b: any, i: number) => {
                ...
                return (
                  <motion.div key={b.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
                    ...
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-600/20 to-teal-600/20 flex items-center justify-center flex-shrink-0 text-sm font-bold text-emerald-300">
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      ...
                      {b.accessReleasedAt && (
                        <button
                          ...
                          title="Revoga a liberação — o paciente volta a ser bloqueado se tiver fatura em aberto">
              ...
              {unbilledPatients.map((s: any, i: number) => {
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input type="number" min="0" step="0.01" value={monthlyPrice}
                    ...
                    placeholder="ex: 150,00" className="input-field flex-1" />
                </div>
              </div>
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input type="number" min="0" step="0.01" value={annualPrice}
                    ...
                    placeholder="ex: 1400,00" className="input-field flex-1" />
                </div>
